...
proc forwardPropagation(network: Network, input: seq[float]): seq[float] =
    var inputs = input
    for i in 0..<network.len:
        var nextInputs = newSeq[float]()
        for j in 0..<network[i].len:
            let activationResult = sigmoid(activation(network[i][j].weights,
                    inputs,
                    network[i][j].bias))
...
proc forwardPropagation(network: var Network, input: seq[float]): seq[float] =
    ...
    for i in 0..<network.len:
        var nextInputs = newSeq[float]()
        for j in 0..<network[i].len:
            network[i][j].output = sigmoid(activation(network[i][j].weights,
                    inputs,
                    network[i][j].bias))
...
proc backpropagation(network: var Network, expectedLabels: seq[float]) =
    ...
    for i in countdown(network.high, 0):
        ...
        for j in 0..<network[i].len:
            ...
            if i == network.high:
                errors.add(expectedLabels[j] - network[i][j].output)
            else:
                var error = 0.0
                for neutron in network[i + 1]:
                    error += neutron.weights[j] * neutron.delta
                errors.add(error)
        for j in 0..<network[i].len:
            network[i][j].delta = errors[j] * derivative(network[i][j].output)
    ...
    for i in 0..<network.len:
        ...
        for j in 0..<network[i].len:
            for k in 0..<inputs.len:
                network[i][j].weights[k] += (alpha * network[i][
                        j].delta * workingInputs[k])
...
proc trainBNN*(network: var Network, trainingData: seq[seq[float]],
    ...
    for epoch in 0..<epochs:
        ...
        for i in 0..<trainingData.len:
            let outputs = forwardPropagation(network, trainingData[i])
            var expected = newSeq[float](network[network.high].len)
            expected.fill(0.0)
...
proc classify*(network: Network, input: seq[float]): seq[float] =
    return forwardPropagation(network, input)
